...
except for the rownames, which contains the Ensembl gene ID. This is
...
![](Lab-Class13--DESeq-lab-_files/figure-commonmark/unnamed-chunk-14-1.png)
...
![](Lab-Class13--DESeq-lab-_files/figure-commonmark/unnamed-chunk-15-1.png)
...
![](Lab-Class13--DESeq-lab-_files/figure-commonmark/unnamed-chunk-16-1.png)
...
transformations like base10 or natural log but we most often prefer log2
units. These units are much easier to understand than using ln or log 10
...
The `arr.ind=TRUE` argument in the `which()` function returns an array
indices in which genes and samples have zero counts. Calling unique()
and taking the first column output which is IDs ensures we don’t double
count zero counts
...
No, because it does not state the significance yet. The results can be
...
Volcano Plot. This ia common type of summary figure that keeps both our
...
![](Lab-Class13--DESeq-lab-_files/figure-commonmark/unnamed-chunk-33-1.png)
...
![](Lab-Class13--DESeq-lab-_files/figure-commonmark/unnamed-chunk-34-1.png)
...
\##8. Adding annotation data Our result table so far only contains the
Ensembl gene IDs. However, alternative gene names and extra annotation
are usually required for informative interpretation of our results. In
this section we will add this necessary annotation data to our results.
...
We will use the **gage** package along with **pathview** here to geneset
enrichment (aka a pathway analysis) and figure generation respectively.
...
Now we can run `gage()` with this input vector and the geneset we want
to examine for overlap/enrichment…
...
We can view these pathways with out geneset genes highlighted using the
`pathview()` function. E.g. for Asthma I will use the pathway.id as seen
above (hsa05310)
...
```
    ...
    Info: Working in directory /Users/Alvin/Desktop/BIMM143/Week 7/Lab Class13 (DESeq lab)
